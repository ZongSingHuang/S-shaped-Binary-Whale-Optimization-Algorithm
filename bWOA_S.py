...
class bWOA_S():
    def __init__(self, fit_func, num_dim=30, num_particle=20, max_iter=500, 
        ...
        self.X = (np.random.uniform(low=self.x_min, high=self.x_max, size=[self.num_particle, self.num_dim])>0.5).astype(int)
    ...
    def opt(self):
        while(self._iter<self.max_iter):
            ...
            self.X = ((1/(1+np.exp(-self.X)))>np.random.uniform(size=[self.num_particle, self.num_dim])).astype(int)
